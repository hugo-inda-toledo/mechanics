...
<div class="requestsAvailableServices index large-9 medium-8 columns content">
    <h3><?= __('Requests Available Services') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col"><?= $this->Paginator->sort('id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('request_id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('available_service_id') ?></th>
        ...
        <tbody>
            <?php foreach ($requestsAvailableServices as $requestsAvailableService): ?>
            <tr>
                ...
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $requestsAvailableService->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $requestsAvailableService->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $requestsAvailableService->id], ['confirm' => __('Are you sure you want to delete # {0}?', $requestsAvailableService->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
